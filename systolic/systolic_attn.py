...
def test_systolic_attention_4x4():
    np_type = get_np_struct_type(32)
    def serialize_to_uint32(matrix):
        return matrix.flatten().view(np_type).copy()
    ...
    if hls.is_available("vitis_hls"):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            modhw(Q_packed, K_packed, S_out, Max_out)
            S_sim = deserialize_S(S_out.view(np.int8))

            np.testing.assert_allclose(S_sim, S_golden, atol=1e-5)
            np.testing.assert_allclose(Max_out, Max_golden, atol=1e-5)
            print("pass")
