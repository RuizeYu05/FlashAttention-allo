...
CONTEXT_LENGTH = 64
HIDDEN_SIZE    = 32
BLOCK_T        = 8          # PE-array dimension AND tc-block size
...
def top(
    ...
    # ── Intra-array streams ────────────────────────────────────────────────
    fifo_Q: Stream[float32[HEAD_DIM], 1024][P0, P1]   # Q flows left → right
    fifo_K: Stream[float32[HEAD_DIM], 1024][P0, P1]   # K flows top  → bottom
    fifo_V: Stream[float32[HEAD_DIM], 1024][P0, P1]   # V flows top  → bottom
 
    # Flash Attention state (strictly left → right, no feedback)
    fifo_m: Stream[float32,          1024][P0, P1]    # running max
    fifo_d: Stream[float32,          1024][P0, P1]    # running normaliser
    fifo_o: Stream[float32[HEAD_DIM],1024][P0, P1]    # running output
 
    # ── Boundary I/O streams ──────────────────────────────────────────────
    # One FIFO per row for Q (left boundary)
    fifo_in_Q: Stream[float32[HEAD_DIM], 1024][P0 - 2]
    # One FIFO per column for K/V; column j+1 gets tc block j
    fifo_in_K: Stream[float32[HEAD_DIM], 1024][P1 - 2]   # depth 4 = BLOCK_T
    fifo_in_V: Stream[float32[HEAD_DIM], 1024][P1 - 2]
    # One FIFO per row for output (right boundary)
    fifo_out:  Stream[float32[HEAD_DIM], 1024][P0 - 2]
        ...
        with allo.meta_else():
            for b, h in allo.grid(BATCH_SIZE, NUM_HEADS):
                for tr in range(0, CONTEXT_LENGTH, BLOCK_T):
                    ...
                    for k in range(BLOCK_T):
                        kv: float32[HEAD_DIM] = fifo_K[i, j].get()
