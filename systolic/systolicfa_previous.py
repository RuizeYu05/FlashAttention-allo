...
def get_kv_tiled_flash_attention(
    ...
    def top(
        ...
        # ---- inter-PE streams ----
        # inter-PE depths cut to 16 (was 64/32): shrinks SRL/LUTRAM at its source so
        # a higher slrUtilDelta can legalize+compact. Capacity-safe: L->R lead<=16 ->
        # req (NCOLS-1)*16*BS=96 << fifo_in_K=512. Depth never changes put/get balance.
        fifo_Q: Stream[float16[HEAD_DIM], 16][P0, P1]   # Q,          left -> right
        fifo_K: Stream[float16[HEAD_DIM], 16][P0, P1]   # K,          top  -> bottom
        fifo_V: Stream[float16[HEAD_DIM], 16][P0, P1]   # V,          top  -> bottom
        fifo_m: Stream[float16, 16][P0, P1]             # running max m,  left -> right
        fifo_d: Stream[float16, 16][P0, P1]             # running denom d, left -> right
        fifo_o: Stream[float16[HEAD_DIM], 16][P0, P1]   # running out  o, left -> right
...
def run_test_with_params(BATCH_SIZE, CONTEXT_LENGTH, HIDDEN_SIZE, NUM_HEADS, NROWS, NCOLS, BS, mode="csyn"):
    ...
    if hls.is_available("vitis_hls"):
        ...
        hls_mod = s.build(target="vitis_hls", mode=mode, project="/scratch/ry375/restructure_head8_seq1024_hw.prj")
        if mode != "csyn":
            hls_mod(Q, K, V, B_out)
...
if __name__ == "__main__":
    ...
    REAL = dict(BATCH_SIZE=1, CONTEXT_LENGTH=1024, HIDDEN_SIZE=32, NUM_HEADS=4,
                NROWS=4, NCOLS=4, BS=2)   # WORKING U280 bitstream config: HEAD_DIM=8 (routable bus width<=8) + slrUtilDelta=0.15
