...
def test_large_scale_gemm():
    ...
    C_golden = S_golden - row_max

    sim_mod = df.build(top, target="simulator")
    sim_mod(A_hw, B_hw, C_hw)
    C_out = C_hw.reshape((M,N))
    np.testing.assert_allclose(C_out, C_golden, atol=1e-5)
    ...
    if hls.is_available("vitis_hls"):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            np.testing.assert_allclose(C_emu_out, C_golden, atol=1e-5)
